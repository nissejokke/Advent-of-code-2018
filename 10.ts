...
class NightSky {
    ...
    buildMap() {
        ...
        for (let star of stars) {
            map[this.getKey(star.pos.x, star.pos.y)] = star;
        }
    ...
    isMessage(): Vector[] {
        ...
        const findInARow = (positions:Vector[], map, minInARow:number): Vector[] => {

            let minx = Math.min(...positions.map(v => v.x));
            let maxx = Math.max(...positions.map(v => v.x));
            let miny = Math.min(...positions.map(v => v.y));
            let maxy = Math.max(...positions.map(v => v.y));

            let hits = [];
            for (let y = miny; y <= maxy; y++) {
                let n = 0;
                for (let x = minx; x <= maxx; x++) {
                    if (map[this.getKey(x, y)])
                        n++;
                    else
                        n = 0;

                    if (n >= minInARow)
                        hits.push(new Vector(x, y));
                }
            }
        ...
        let hitmap = {};
        for (let star of stars) {
            let x = Math.ceil(star.pos.x / parts);
            let y = Math.ceil(star.pos.y / parts);
            let key = x + ',' + y;
            hitmap[key] = hitmap[key] || 0;
            hitmap[key]++;
        ...
        if (hits.length) {
            let found = findInARow(hits, map, 5);
            if (found.length) {
                const d = 20;
                let minx = Math.min(...found.map(v => v.x));
                let maxx = Math.max(...found.map(v => v.x));
                let miny = Math.min(...found.map(v => v.y));
                let maxy = Math.max(...found.map(v => v.y));
                return [new Vector(minx - d, miny - d), new Vector(maxx + d, maxy + d)];
            }
        }
    ...
    print(bounds: [Vector, Vector]) {

        let map = this.buildMap();

        let topx = bounds[0].x;
        let topy = bounds[0].y;
        let bottomx = bounds[1].x;
        let bottomy = bounds[1].y;

        for (let y = topy; y <= bottomy; y++) {
            let row = [];
            for (let x = topx; x <= bottomx; x++) {
                let star = map[this.getKey(x, y)];
                row.push(star ? '#' : '.');
            }
...
let sky = new NightSky(stars);
let bounds;
let docontinue = true;
while (docontinue) {
    sky.iterate(1);
    while (!(bounds = sky.isMessage())) {
        sky.iterate(1);
    }
    sky.print(bounds);
