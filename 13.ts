...
class CartMap {
    ...
    moveCarts(carts:Cart[]) {
        ...
        for (let cart of carts) {
            ...
            if (colls.length > 0) {
                collisions = collisions.concat(colls);
                return collisions;
            }
        }
        return collisions;
...
for (let i = 0; i < 2000; i++) {
    ...
    let crasches = map.moveCarts(carts);
    if (printmap) {
        map.print(carts);
        console.log('--------------');
    }
    if (crasches.length) {
        for (let crasch of crasches)
            console.log('boom', crasch[0].x + ',' + crasch[0].y);
        break;
    }
}