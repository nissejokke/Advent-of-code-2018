...
for (let y = ymin; y <= ymax; y++) {
    ...
    for (let x = xmin; x <= xmax; x++) {
        let pointDistances = points.map(point => {
            return { 
                point, 
        ...
        let key = x + ',' + y;
        if (pointDistances[0].distance === pointDistances[1].distance)
            map[key] = '.';
        else {
            map[key] = pointDistances[0].point.name;
            if (y === ymin || y === ymax || x === xmin || x === xmax)
                infinites[map[key]] = true;
        }
        row += map[key];
    }
}
...
for (let point of points) {
    if (!infinites[point.name])
        scores.push({
            name: point.name,
            size: Object.values(map)
                .filter(val => val === point.name)
                .length
        });
}

let largest = scores.sort((a, b) => b.size - a.size)[0];
